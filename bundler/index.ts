...
const minify = async (code: string, type: "css" | "js"): Promise<string> => {
  switch (type) {
    case "css":
...
const defArr: Array<string> = []
const jsArr: Array<string> = []
...
type ComponentType = 'extension' | 'new';
...
type ValidComponentMetadata = 
  | (Omit<ComponentMetadata, 'type'> & { type: 'new' })
  | (Omit<ComponentMetadata, 'type' | 'extends'> & { type: 'extension', element: string, extends: string });
for (let file of readdirSync(join(__dirname, '..'), { recursive: true })) {
    ...
    if (file.endsWith("component.json")) {
        const metadata: ValidComponentMetadata = JSON.parse(readFileSync(join(__dirname, '..', file), "utf-8"))
